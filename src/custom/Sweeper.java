...
public class PrivateKeys {
    public static void main(String[] args) throws Exception {
	    ...
	    var private_key = '5Kat7WkPTrtjyvx1yr1YeX3nTMzA7JvczkHHWyNziyJV5FQTqHW';
	    var destination = '15YxvifEx2KrpiyRqUFGMBGQuuAUQvpSbe';
	    var amount = 0.01;
            ECKey key;
            if (private_key.length() == 51) {
                DumpedPrivateKey dumpedPrivateKey = new DumpedPrivateKey(params, private_key);
                key = dumpedPrivateKey.getKey();
            } else {
                BigInteger privKey = Base58.decodeToBigInteger(private_key);
                key = new ECKey(privKey);
            }
            System.out.println("Address from private key is: " + key.toAddress(params).toString());
            // And the address ...
            Address destination = new Address(params, );
		...
		wallet.sendCoins(peerGroup, destination, Utils.toNanoCoins(amount));
	    }
